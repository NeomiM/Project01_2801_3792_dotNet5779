/*
 
BE
 1 V what is a struct
2 V how to do global enums and add to a class
3 add what to do if id is wrong/ not in the right format
5 V add a toString for the tester class
6 think of more properties for tester and trainee: picture, how many stars, option to write review
7 V configureation: make global firstTestID for the first test number
                //check if it's all numbers- 8/9 numbers
                //check for letters,and ammount of numbers in the id

 DAL
 1 V how to get the lists from DS->DataSource.cs
 2 implement the functions in Dal_imp
 3 V add check for id if exist
 4 V make "printer number" for testId from usercontrol (automatially updates number from the first number in configuration in BE)
 5 make uopdates for people and test
 DS
V make netunim for when the program starts
  

 Questions:
 1V how poublic/private should the classes be?
 2V is there a function that gets the list of people? (DAL)
 3 V what directios are the projects for references?
 4 is it enough to just add a mishtneh of a person or to use params[] or deteils
  
  
  4/12
  finish vheck on id - Beila
  finish scedual -Beila
  V to srtings - how to print nicely- Neomi
  get beila connected -Neomi

 V what  to do for exceptions.
  
 
 10/12
 1 finish dal imp - add exceptions
 2 work on adding a test with the right tester -ibl_imp
 3 work out what distance for where in Addtest -ibl_imp


 neomi: 
 V what is the difference between argumant exceptoion and exception
 finish writing bl functions
 add functions to bl to make it all smaller
 V add parse to check id
add email
 Beila:
  dont worry be happy
 add excetions to DAL_imp
 add checks in PL 

 




  */